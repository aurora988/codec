function decodeUplink (input) {
  ...
  var sizeEnumObj = {
    valveStatus: {
      ...
      1: "close valve",
      2: "dredge valve",
      3: "Turn on the timing dredging valve function",
      4: "Turn off the timing dredging valve function",
    },
    PN: { // L/P
      0: 0.5,
      1: 1,
    ...
    meterStatusAlarm: {
      ...
      2: {
        1: "has a history of magnetic attacks",
        5: "disassembly alarm",
        6: "backwater alarm",
        7: "water intrusion alarm",
      },
    },
  };
  // the test data
  // var input = {
  //   bytes: [36, 25, 37, 22, 17, 34, 51, 68, 20, 3, 27, 0, 18, 1, 28, 22, 2, 22, 23, 8, 52, 11, 0, 0, 0, 0, 26, 0, 59, 51, 132, 2, 35, 4, 68]
  // };

  tempStr = input.bytes.map(function (byte) {
  ...
  funcEnumObj = {
    ...
    '1C': function (tempStr, sizeEnumObj, tempObjOne) {
      fc = 6 * 2 + 2;
      tempObjOne.endDeviceTime = "20" +
        ("00" + String(parseInt(tempStr.slice(2, 4), 16))).slice(-2) + "-" +
        ("00" + String(parseInt(tempStr.slice(4, 6), 16))).slice(-2) + "-" +
        ("00" + String(parseInt(tempStr.slice(6, 8), 16))).slice(-2) + " " +
        ("00" + String(parseInt(tempStr.slice(8, 10), 16))).slice(-2) + ":" +
        ("00" + String(parseInt(tempStr.slice(10, 12), 16))).slice(-2) + ":" +
        ("00" + String(parseInt(tempStr.slice(12, 14), 16))).slice(-2)

      return [tempStr.slice(fc), tempObjOne]
    },
    '16': function (tempStr, sizeEnumObj, tempObjOne) {
      fc = 4 * 2 + 2;
      tempObjOne.serialNumber = parseInt(tempStr.slice(2, fc), 16);
      return [tempStr.slice(fc), tempObjOne]
    },
    ...
    '33': function (tempStr, sizeEnumObj, tempObjOne) {
      ...
      var t2ObjTMP2 = sizeEnumObj.meterStatusAlarm[2];
